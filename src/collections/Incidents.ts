...
async function createIncidentNotificationDeferred(data: DeferredNotification) {
  try {
    ...
    if (data.isInitial) {
      ...
      const template =
        smsSettings.templateIncidentNew ||
        "[{{siteName}}] 🚨 INCIDENT: {{title}} | {{status}} | {{message}} | {{url}}";
      ...
      const template =
        smsSettings.templateIncidentUpdate ||
        "[{{siteName}}] 📢 {{title}} | {{status}} | {{message}} | {{url}}";
      smsBody = interpolateTemplate(template, templateVars);
    ...
    const notification = await payload.create({
      ...
      data: {
        title: data.isInitial
          ? `[New Incident] ${data.title}`
...
export function getIncidentStatus(
  updates: Array<{ status?: string }> | null | undefined,
): IncidentStatus {
...
export function getIncidentResolvedAt(
  updates: Array<{ status?: string; createdAt?: string }> | null | undefined,
): string | null {
    ...
    afterChange: [
      async ({ doc, operation, previousDoc, req }) => {
        ...
        if (operation === "create" && currentUpdates.length > 0) {
          ...
          const statusLabel =
            statusLabels[firstUpdate.status] || firstUpdate.status || "Investigating";
